...
def render_set(sys_param, model_path, name, iteration, views, background, separate_sh):
    # 渲染图片存储路径
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        # 渲染图像
        render_image = render(view, gaussian_model, sys_param, background, separate_sh=separate_sh)['render']
        # 标准图像rgb
...
def render_sets(sys_param, gaussian_model: GaussianModel, scene:Scene, separate_sh: bool):
    with torch.no_grad():
        ...
        if not sys_param['skip_train']:
            render_set(sys_param, sys_param['model_path'], "train", scene.loaded_iter, scene.getTrainCameras(), background, separate_sh)
        # 渲染测试新视图
        if not sys_param['skip_test']:
            render_set(sys_param, sys_param['model_path'], "test", scene.loaded_iter, scene.getTestCameras(), background, separate_sh)
