...
def get_expon_lr_func(lr_init, lr_final, lr_delay_steps=0, lr_delay_mult=1.0, max_steps=1000000):
    def helper(step):
        ...
        lr_init_val = lr_init.detach().cpu().numpy() if torch.is_tensor(lr_init) else lr_init
        lr_final_val = lr_final.detach().cpu().numpy() if torch.is_tensor(lr_final) else lr_final
        t_val = t.detach().cpu().numpy() if torch.is_tensor(t) else t
        log_lerp = np.exp(np.log(lr_init_val) * (1 - t_val) + np.log(lr_final_val) * t_val)
