...
def train(sys_param, gaussian_model: GaussianModel, scene: Scene):
    ...
    for iteration in range(first_iter, sys_param['iterations_MAX'] + 1):
        ...
        with torch.no_grad():
            ...
            ######################## 日志记录和模型保存 ########################
            ssim = 1.0 - ssim_value
            training_report(tb_writer, iteration, ssim, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), sys_param['test_iterations'], scene, render, (sys_param, background, SPARSE_ADAM_AVAILABLE))
...
def training_report(tb_writer, iteration, ssim, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, sys_param):
    if tb_writer:
        tb_writer.add_scalar('train_loss_patches/l1_loss', Ll1.item(), iteration)
        tb_writer.add_scalar('train_loss_patches/total_loss', loss.item(), iteration)
        tb_writer.add_scalar('train_loss_patches/ssim_loss', ssim.item(), iteration)
    ...
    if iteration in testing_iterations:
        ...
        if tb_writer:
            tb_writer.add_histogram("scene/opacity_histogram", scene.gaussian_model.get_opacity, iteration)
            tb_writer.add_scalar('total_points', scene.gaussian_model.get_xyz.shape[0], iteration)
        torch.cuda.empty_cache()
